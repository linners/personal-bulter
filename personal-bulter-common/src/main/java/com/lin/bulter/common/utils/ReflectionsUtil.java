...
public class ReflectionsUtil {
	 ...
	 * @return
	 */
	public static Object getClassInstance(Map context, String className, String... parameterizedTypes) throws Exception {
		// 反射获得类(若接口，则返回实现类）
		Class<?> aClass = getClassImplName(className);
		Map<String, String> parameterizedTypeMap = new HashMap<>();
		TypeVariable<? extends Class<?>>[] typeParameters = aClass.getTypeParameters();
		...
		if (result != null && result.length > 0) {
			for (Field field : result) {
				...
				System.out.print("field name: " + fieldName);
				System.out.println("");
				String fieldRealVal = context.get(fieldName).toString();
				if (typeName.equals("java.lang.String")) {
